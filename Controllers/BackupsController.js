...
module.exports = class BackupsController {
    ...
    static async saveBackup(req,res){
        try {
            ...
            if(quantity > 3) {
                ...
                const toDelete= await Backup.find().select('-data -date').limit(1)

                console.log('to delete-------',toDelete._id)

                const deletion = await Backup.deleteOne({_id: toDelete[0]._id})


                if(!deletion.acknowledged || deletion.deletedCount != 1){
                    Logger.warn({message: 'A problem was detected while deleting old backups', deletion: deletion})
                } 
            }
            ...
            RequestHandler.sendSuccess(req.requestId, res, {isSuccessful: true})
    ...
    static async restoreBackup(req,res){
        try {
            ...
            if(!latestBackup){
                Logger.warn('request for latest backup returned a falsey value')
                RequestHandler.throwError(404,'request for latest backup returned a falsey value')()
            }else if (latestBackup.length < 1) {
                Logger.warn('request for latest backup an empty array instead of one backup object')
